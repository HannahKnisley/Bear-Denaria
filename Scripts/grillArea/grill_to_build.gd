...
func slideOutAnimate(delta):
	if self.global_position < (defaultPos+Vector2(295,0)) and out:
		self.translate(((defaultPos+Vector2(300,0))-self.global_position)*delta*10)
		myPatty.move_and_collide(self.global_position-myPatty.global_position)
	else:
		out = false
		myPatty.set_collision_mask_value(1, true)
		myPatty.set_collision_mask_value(2, false)
